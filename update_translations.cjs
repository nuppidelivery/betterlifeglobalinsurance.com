...
Object.keys(keysToAdd).forEach(key => {
    const ptText = keysToAdd[key].pt;
    // We use a regex that matches the exact text in a tag without data-i18n, and adds it.
    // For simplicity, we can just replace >TEXT< with > data-i18n="key">TEXT<
    // Or replace `>Acidentes de trânsito anuais nos EUA</a>` with ` data-i18n="stat_1_desc">Acidentes de trânsito anuais nos EUA</a>`
    ...
    // For standard text replacements
    const safeRegex = new RegExp('([>])\\s*(' + ptText.replace(/[.*+?^$\/{}()|[\]\\]/g, '\\$&') + ')\\s*(<)', 'g');
    indexHtml = indexHtml.replace(safeRegex, (match, p1, p2, p3) => {
        // if the tag before it already has data-i18n, don't break it
        return p1 + p2 + p3;
    });
});

// Since the safeRegex approach doesn't inject into the opening tag easily, let's just do targeted replaces for known tags.
...
Object.keys(keysToAdd).forEach(key => {
    ptTranslationsStr += `          ${key}: "${keysToAdd[key].pt}",\n`;
    enTranslationsStr += `          ${key}: "${keysToAdd[key].en}",\n`;
});
...
// Also need to handle document title translation natively if data-i18n="site_title" works?
// In main.js, the updateText function handles translation.
// Does updateText handle document.title?
// Let's modify updateText to check if el is document.title
const updateTextFunc = /function updateText\(\)\s*{[\s\S]*?}/;
if (mainJs.match(updateTextFunc)) {
    // Modify updateText to also handle title
    mainJs = mainJs.replace(/document\.querySelectorAll\('\\[data-i18n\\]'\)\.forEach\\(el => {/g, 
...
// Add the site_title key back if we missed it
if (!mainJs.includes("site_title:")) {
